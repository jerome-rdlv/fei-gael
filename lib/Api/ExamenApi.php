...
class ExamenApi
{
     ...
     *
     * @param  \Fei\Gael\Model\AddNewCandidatsRequest $addNewCandidatsRequest La liste des nouveaux candidats avec leurs informations (required)
     *
     * @throws \Fei\Gael\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return \Fei\Gael\Model\CandidatCree[]|\Fei\Gael\Model\ResponseBase
     */
    public function addNewCandidats($addNewCandidatsRequest)
    {
        list($response) = $this->addNewCandidatsWithHttpInfo($addNewCandidatsRequest);
        return $response;
    }
     ...
     */
    public function addNewCandidatsWithHttpInfo($addNewCandidatsRequest)
    {
        $request = $this->addNewCandidatsRequest($addNewCandidatsRequest);
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function addNewCandidatsAsync($addNewCandidatsRequest)
    {
        return $this->addNewCandidatsAsyncWithHttpInfo($addNewCandidatsRequest)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function addNewCandidatsAsyncWithHttpInfo($addNewCandidatsRequest)
    {
        $returnType = '\Fei\Gael\Model\CandidatCree[]';
        $request = $this->addNewCandidatsRequest($addNewCandidatsRequest);
     ...
     * @return \GuzzleHttp\Psr7\Request
     */
    public function addNewCandidatsRequest($addNewCandidatsRequest)
    {
        // verify the required parameter 'addNewCandidatsRequest' is set
        if ($addNewCandidatsRequest === null || (is_array($addNewCandidatsRequest) && count($addNewCandidatsRequest) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $addNewCandidatsRequest when calling addNewCandidats'
            );
        }
        ...
        // for model (json/xml)
        if (isset($addNewCandidatsRequest)) {
            if ($headers['Content-Type'] === 'application/json') {
                $httpBody = \GuzzleHttp\json_encode(ObjectSerializer::sanitizeForSerialization($addNewCandidatsRequest));
            } else {
                $httpBody = $addNewCandidatsRequest;
            }
        } elseif (count($formParams) > 0) {
            if ($multipart) {
                $multipartContents = [];
     ...
     * @param  string $codeSession Le code session pour laquelle on veut récupérer la liste des candidats (required)
     * @param  string $codeCentre Le numéro du centre dans GAEL tel qu&#39;affiché sur l&#39;onglet &#39;Statut du centre&#39; (required)
     * @param  NiveauExamen $niveau (required)
     *
     * @throws \Fei\Gael\ApiException on non-2xx response
     * @throws \InvalidArgumentException
